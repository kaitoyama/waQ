...
export default function ControlPage() {
  ...
  const load = async () => {
    const response = await backendFetch(`${backendURL}/controls/broadcasts`)
  ...
  const transition = async (broadcast: Broadcast, action: "start" | "stop") => {
    ...
    setMessage("")
    try {
      const response = await backendFetch(`${backendURL}/controls/broadcasts/${encodeURIComponent(broadcast.id)}/${action}`, { method: "POST" })
